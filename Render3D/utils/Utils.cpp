...
int MakeMeshAndMaterialByObj(Render3D::Vertex **mesh, unsigned long &mesh_num, int **materialIds, unsigned long &materialIdsNum, const char *name)
{
	...
	int start_material_cnt = materials.size();
...

	{
		...
		for (i = 0; i < num_materials; i++)
		{
			...
			m.indexOfRefraction = tm.ior;
			m.dissolve = tm.dissolve;
			m.illumination = tm.illum;
			m.pad0 = tm.pad0;

			m.ambientTexId = -1;
			m.diffuseTexId = -1;
			m.specularTexId = -1;
			m.specularHighlightTexId = -1;
			m.bumpTexId = -1;
			m.displacementTexId = -1;
			m.alphaTexId = -1;

			m.ambientTexname = NULL;
			m.diffuseTexname = NULL;
			m.specularTexname = NULL;
			m.specularHighlightTexname = NULL;
			m.bumpTexname = NULL;
...

	{
		...
		*mesh = (Render3D::Vertex*)malloc(sizeof(Render3D::Vertex) * num_triangles * 3);

		materialIdsNum = num_triangles;
		*materialIds = (int*)malloc(sizeof(int) * (materialIdsNum));
		for (i = 0; i < attrib.num_face_num_verts; i++)
		{
			size_t f;
			assert(attrib.face_num_verts[i] % 3 == 0); /* assume all triangle faces. */
			(*materialIds)[i] = attrib.material_ids[i];
			if ((*materialIds)[i] >= 0)
				(*materialIds)[i] += start_material_cnt;
			...
			for (f = 0; f < (size_t)attrib.face_num_verts[i] / 3; f++)
			{
				...
				for (k = 0; k < 3; k++)
				{
					(*mesh)[3 * i + k].pos = Math3D::Vector4(v[k][0], v[k][1], v[k][2], 1);
					(*mesh)[3 * i + k].normal = Math3D::Vector4(n[k][0], n[k][1], n[k][2], 0);

					(*mesh)[3 * i + k].tc.u = t[k][0];
					(*mesh)[3 * i + k].tc.v = t[k][1];
					...
					(*mesh)[3 * i + k].color = Base3D::Color((c[0] * 0.5f + 0.5f), (c[1] * 0.5f + 0.5f), (c[2] * 0.5f + 0.5f), 1.0f);
				}
			}
...
{
	uint32_t **mipmaps = NULL;
	int num_mip_levels = LogBase2OfX(texture.width) + 1;
	texture.datasLen = num_mip_levels;
	mipmaps = (uint32_t **)malloc(num_mip_levels * sizeof(uint32_t *));
	...
	for (int mip_level = 1; mip_level < num_mip_levels; mip_level++)
	{
		...
		mipmaps[mip_level] = (uint32_t*)malloc(mip_width * mip_height * sizeof(uint32_t));
		uint32_t *src_buffer = mipmaps[mip_level - 1];
		uint32_t *dest_buffer = mipmaps[mip_level];
		for (int x = 0; x < mip_width; x++)
		{
			for (int y = 0; y < mip_height; y++)
			{
				...
				uint32_t c = src_buffer[(x * 2 + 0) + (y * 2 + 0)*mip_width * 2];
				b0 = c & 0xff;
				g0 = (c >> 8) & 0xff;
				r0 = (c >> 16) & 0xff;
				a0 = (c >> 24) & 0xff;
				...
				r_avg = (uint32_t)(0.5f + gamma*(r0 + r1 + r2 + r3) / 4);
				g_avg = (uint32_t)(0.5f + gamma*(g0 + g1 + g2 + g3) / 4);
				b_avg = (uint32_t)(0.5f + gamma*(b0 + b1 + b2 + b3) / 4);
				a_avg = (uint32_t)(0.5f + gamma*(b0 + b1 + b2 + b3) / 4);
...
int LoadPngImage(const char *name, unsigned int **bits, unsigned int &width, unsigned int &height)
{
	FILE *fp;
	png_structp png_ptr;
	png_infop info_ptr;
...
int MakeTextureByPng(const char *name, bool mipmap)
{
	...
	int res = LoadPngImage(findPos == NULL ? name : trueName, &data, texture.width, texture.height);
	if (res == 0)
	{
